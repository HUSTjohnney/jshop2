...
public class JSHOP2GUI extends JFrame {
    ...
    // DefaultMutableTreeNode's, and their indices represent the order in which they
    // were
    // visited.
    ...
    // The invisible root node that is the parent for the first node displayed in
    // the tree
    ...
    // The text area where information about the current state and preconditions is
    // displayed.
    ...
    // A hashtable of DefaultMutableTreeNode references used to keep track of the
    // nodes in the tree.
    ...
    // inefficient for
    // large domains that backtrack a lot. In those cases, a search for a particular
    // node within
    // treeNodeReferences may waste a lot of time looking over nodes that have been
    // deleted.
    ...
    // Holds the dimensions of the screen. Used to center all frames and dialog
    // boxes.
    ...
    private boolean runOneStep() {
        ...
        if (iterator < planStepList.size()) {
            ...
            // trying a task
                ...
                // by a "trying" step, and it's implemented to enable dynamic leaf node
                // tracking.
                if (iterator - 2 >= 0)
            ...
            // Ensures that after a REDUCED and STATECHANGED step, a 'valueChanged' event
            // will be generated
    ...
    /**
     * Helper function to runOneStep().
     * This function adds children to existing nodes and marks them if they are
     * ordered.
     */
    private DefaultMutableTreeNode processReduced(PlanStepInfo step, ArrayList<Node> toAdd) {
        ...
        // The term "parent" here is referring to the current node (parent as in parent
        // of
     ...
     * 'children' field
     * of the 'step' parameter. Each element of 'children' represents a TaskList
     * that encapsulates
     * a goal task. Calling getTask() on each TaskList gives access to the TaskAtom
     * that represents
     * the goal task. If getTask() returns null, then that means the goal task is
     * encapsulated
    ...
    private void processBacktracking(PlanStepInfo step) {
        ...
        // If backtracking from a leaf node, check to see if this leaf node was the most
        // recent
        // leaf node to be added to 'leafNodes'. If true, then delete this leaf node's
        // tag and
        // remove it from 'leafNodes'.
        // <Note:> I'm assuming that whenever you backtrack from a leaf node, that leaf
        // node will
        // always have the highest tag value in 'leafNodes'. I'm not certain if this is
        // always the case.
        else {
