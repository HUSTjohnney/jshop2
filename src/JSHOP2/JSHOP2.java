...
public class JSHOP2 {
   ...
   *
   * @param chosenTask
   *                   the task list chosen to look for the next task atom to
   *                   achieve.
   *                   This variable is usually set to the whole task network
   *                   unless
   *                   there is a method that is chosen to decomopose a task, and
   *                   the
   *                   decomposition of that task has not gone all the way down to
   *                   an
   *                   operator. In that case, this variable will be set to the
   *                   task
          ...
          if (v.binding != null) {
            ...
            while ((v.nextB = v.p.nextBinding()) != null) {
              ...
              if (v.o[v.j].apply(v.nextB, state, v.delAdd)) {
                ...
                newStep.operatorInstance = v.o[v.j].getHead().applySubstitution(v.nextB)
                    .toString(JSHOP2.getDomain().getPrimitiveTasks());
                planStepList.add(newStep);
